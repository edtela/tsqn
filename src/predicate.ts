...
export function evalPredicate<T>(value: T, predicate: Predicate<T>): boolean {
  ...
  if (result) {
    ...
    return Object.keys(predicate).every((key: any) => evalPredicate(value[key], predicate[key]));
  }
  return result;
}
  ...
  switch (operator) {
    ...
    case ALL:
      if (value !== null && valueType === "object") {
        return Object.keys(value).every((key) => evalPredicate(value[key], condition));
      }
      return false;
    case SOME:
      if (value !== null && valueType === "object") {
        return Object.keys(value).some((key) => evalPredicate(value[key], condition));
      }
      return false;
  }
  return false;
